...
def sell_product():
    ...
    stock_alert = False
    threshold_qty = 0.2 * (current_qty + quantity)  # original qty before this sale
    if new_qty <= threshold_qty:
        stock_alert = True

    conn.commit()
    conn.close()
    print('Remaining quantity:', new_qty)
    return jsonify({
...
def add_account():
    ...
    try:
        cursor.execute("""
            INSERT INTO seller_accounts (sellerName, phoneNumber, vehicleId, driverName)
            VALUES (%s, %s, %s, %s)
