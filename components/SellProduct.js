...
export default function SellProduct({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      const loadCategoriesAndProducts = async () => {
        try {
          const response = await fetch('http://192.168.0.109:3000/getInventory');
  ...
  const handleAddSale = async () => {
    ...
    try {
      const response = await fetch('http://192.168.0.109:3000/sellProduct', {
        method: 'POST',
        headers: {
      ...
      const refreshedInventory = await fetch('http://192.168.0.109:3000/getInventory');
